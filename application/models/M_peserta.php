...
class m_peserta extends CI_model
{
    ...
    public function getBiaya($id_peserta){
        $pendaftaran = $this->db->query("SELECT pendaftaran.id_jalur_pendaftaran,pendaftaran.id_program_studi,pendaftaran.id_tahun_ajaran FROM `peserta` JOIN pendaftaran ON pendaftaran.id_pendaftaran=peserta.id_pendaftaran WHERE peserta.id_peserta = $id_peserta")->row_array();

        $id_jalur = $pendaftaran['id_jalur_pendaftaran'];
        $id_jurusan = $pendaftaran['id_program_studi'];
        $id_tahun = $pendaftaran['id_tahun_ajaran'];

        return $this->db->query("SELECT * FROM `biaya_masuk` WHERE `id_jalur_pendaftaran` = $id_jalur AND `id_program_studi` = $id_jurusan AND `id_tahun_ajaran` = $id_tahun")->result_array();
    }
    ...
    public function cekDaftarUlang($id_peserta){
        $cek = $this->db->get_where('daftar_ulang',['id_peserta' => $id_peserta, 'status' => 'sudah'])->num_rows();
